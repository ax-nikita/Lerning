...
class CreateTicketRequest
{
    ...
    public static function fromArray(array $data): self
    {
        ...
        if (!isset($data['title'], $data['priority'], $data['status'])) {
            throw new RequestValidationException('data must be a valid array');
        }

        // Проверка корректности заголовка
        if (!is_string($data['title'])) {
            throw new RequestValidationException('title must be a string');
        }
        ...
        if (!is_string($data['priority'])) {
            throw new RequestValidationException('priority must be a string');
        }

        $priority = TicketPriority::tryFrom($data['priority']);

        if ($priority === null) {
            throw new RequestValidationException('such a priority is unacceptable');
        }

        // проверка статуса
        if (!is_string($data['status'])) {
            throw new RequestValidationException('status must be a string');
        }

        $status = TicketStatus::tryFrom($data['status']);

        if ($status === null) {
            throw new RequestValidationException('such a status is unacceptable');
        }

        $assigneeId = $data['assigneeId'] ?? null;

        if ($assigneeId !== null && !is_int($assigneeId)) {
            throw new RequestValidationException('assigneeId must be an integer or null');
        }
